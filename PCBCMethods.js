...
function MySet(modulus,...elements) {
    ...
    */
     ...
     */
    this.transpose = function (array = this.set, modulus = this.universe, index = 0,notes = false) {
        return notes? this.noteNames(...array.map(x => this.modulo(x+index,modulus))) : array.map(x => this.modulo(x+index,modulus)); //O(n);
    },
    /**
     * 
     * @param {int} index 
     ...
     */
    this.invert = function (array = this.set,modulus = this.universe,index = 0,notes = false) {
        return notes? this.noteNames(...array.map(x => this.modulo(index-x,modulus))) : array.map(x => this.modulo(index-x,modulus)); //O(n);
        },
    /**
    ...
    * @returns powerset
    */
    this.literal_subsets = (cardinality = null,array = this.set,notes = false) => {   // O(2^n) //4+(2^n) operations. 
        let result = [];
        if (cardinality === null) {
            for (let a = 1; a <= array.length; a++) {
     ...
     */
    this.normal_order = (array = this.set,mod = this.universe,notes = false) => { // Total = O(n^2)
        ...
        while (index > 0) {     //n
            ...
            if (small.length == 1 || index == 0) {
                return notes? this.noteNames(...rotations[small[0]]) : rotations[small[0]];
            }
        ...
        return notes? this.noteNames(...rotations[0]) : rotations[0];    //if rotations.length > 1 all are acceptabe Normal Orders.
    }
    /**
    ...
    * @returns Prime Form
    */
    this.prime_form = (array = this.set,mod = this.universe,notes = false) => { // O(n);
        ...
        while (round < intervals[0].length) { //n-1;
            if (intervals[0][round] < intervals[1][round]) {
                return notes? this.noteNames(...options[0].map(x => this.modulo(x-options[0][0],mod))) : options[0].map(x => this.modulo(x-options[0][0],mod));
            }
            else if (intervals[0][round] > intervals[1][round]) {
                return notes? this.noteNames(...options[1].map(x => this.modulo(x-options[1][0],mod))) : options[1].map(x => this.modulo(x-options[1][0],mod));
            }
     ...
     * @returns Set Class
     */
    this.set_class = (array = this.set,modulus = this.universe,eliminateDuplicates = false,notes = false) => {
        let result = {};
        for (let a = 0; a < modulus; a++) {
            result['T'+a] = this.normal_order(array.map(x => this.modulo(x+a,modulus)),modulus,notes);
            result['I'+a] = this.normal_order(array.map(y => this.modulo(a-y,modulus)),modulus,notes);
        }
    ...
    /**
     * Toggles integer and pitch notation if modulus of set object is 12.
     * @param  {...any} array 
     */
    this.noteNames = (...array) => {
        if (modulus == 12) {
            ...
            let pitches = ['C','C♯/D♭','D','D♯/E♭','E','F','F♯/G♭','G','G♯/A♭','A','A♯/B♭','B'];
                if (dType == 'string') {
                    ...
                    console.error(`Wrong data type of input elements, expected 'number' or 'string'.`)
                }
                return modified;
        }
        else {
            console.error(`Note names unavailable in mod ${modulus}!`)
        }
    }
};
                ...
                for (let [key,value] of Object.entries(literalContains)) {
                    ...
                    butto.addEventListener('mousedown',() => {
                        ...
                        if (clicked == true) {
                            ...
                            grp.add(this.draw.text(`${butto.textContent}`).center(...centroid));
                            document.getElementById('modifiedDisplay').innerHTML = `[${this.setrepSub.normal_order()}] under ${key} = ${butto.textContent}`;
                        }
                        else {
